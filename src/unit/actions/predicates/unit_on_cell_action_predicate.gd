...
func _evaluate(unit: Unit, cell: Vector2i) -> bool:
	...
	if required_unit_count == 1: ## Slight optimization to use any in this case, probably unneeded
			return units.any(
					func(unit_on_cell: Unit) -> bool: return _does_match_predicate(unit_on_cell, unit)
				)
	elif required_unit_count == 0:
		return not units.any(
				func(unit_on_cell: Unit) -> bool: return _does_match_predicate(unit_on_cell, unit)
			)
	else:
		return units.filter(
				func(unit_on_cell: Unit) -> bool: return _does_match_predicate(unit_on_cell, unit)
			).size() >= required_unit_count
...
func _does_match_predicate(unit_on_tile: Unit, testing_unit: Unit) -> bool: 
	return unit_predicate_list.all(
			func(pred: UnitPredicate) -> bool: return pred.test(unit_on_tile, testing_unit) 
		)
